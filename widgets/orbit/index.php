...
// define data and dependencies of widget	
    ...
    public function get_script_depends(): array {
		return [ 'widget-script-orbitRotationScript' ];
	}
...
//define redering of widget
	protected function render(): void {;
        ...
        if ( empty( $settings['orbitingElementsList'] ) ) {
            ...
            $offsetSine = sin($settings['orbitingAngleOffset']['size']);
            $offsetCos = cos($settings['orbitingAngleOffset']['size']);
            
            $orbitingEl = function($i,$x,$y)use($settings, $offsetCos,$offsetSine) {
                //cos(a+b) identity
                // $offsetX = $offsetCos * $x - $offsetSine * $y;
                 //sin(a+b) identity
            //    $offsetY = $offsetCos * $y  +  $offsetSine * $x;

                $src = "http://localhost:10004/wp-content/plugins/Itay-s-Elementor/global/../widgets/orbit/assets/star.png";//getRandomFileFromFolder(__DIR__ . '/assets/defualtStars');
                ...
                " initialCos="<?echo $x; ?>" initialSin="<?echo $y; ?>"> 
            ...
            //----------------draw orbits
             ?>
		<div class="circular-container" rotation_animation="<?php echo $settings['rotationAnimation']; ?>">
            ...
            for($i=1; $i <= floor(($n-1)/2); $i++){ 
               ...
               $orbitingEl($n-$i, $currentCos,(-1) * $currentSine); //its symetric conjugate
